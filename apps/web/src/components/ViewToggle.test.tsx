...
describe("ViewToggle", () => {
  ...
  it("is a radio group of the two views", () => {
    ...
    expect(screen.getAllByRole("radio").map((el) => el.textContent)).toEqual(["List", "Migrations"]);
  });

  it("marks the current mode checked, and the other one not", () => {
    render(<ViewToggle mode="migrations" onChange={vi.fn()} />);
  ...
  it("reports the mode that was chosen", () => {
    ...
    expect(onChange).toHaveBeenCalledWith("migrations");
  });
  ...
  // Two options today, three (List, Graph, Migrations) until the graph view
  // was decommissioned 2026-09-05 (docs/graph-removal-brief.md) -- the
  // invariant this asserts is "exactly one", not "the other of two", and it
  // held unchanged across both counts.
  ...
  it("treats ArrowDown/ArrowUp the same as ArrowRight/ArrowLeft", () => {
    ...
    fireEvent.keyDown(migrations, { key: "ArrowUp" });
    expect(document.activeElement).toBe(list);
  });
  ...
  it("leaves focus on the checked option when the parent declines the change", () => {
    ...
    expect(screen.getByRole("radio", { name: "Migrations" }).getAttribute("tabindex")).toBe("-1");
  });
