...
// before the first "-" -- see @dagstree/cli's view-payload.ts, which
...
/** Ordinal compare (plain `<`), not a locale-aware collation -- matches the sort @dagstree/cli's workspace-scan.ts already uses, for the same reason: stable regardless of the host's ICU data. */
 ...
 * (@dagstree/schema enforces it), so the order is stable and reproducible
