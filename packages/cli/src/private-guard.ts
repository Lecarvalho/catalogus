...
// section 5's "Schema guardrail"): catalogus.yaml must never carry cost,
// billing, or account-shaped data.
//
// This is deliberately scoped to *flag/property names*, mirroring exactly
// what @catalogus/schema itself checks -- its patternProperties deny rule
...
//
// What genuinely mirrors the schema's actual boundary: catalogus add/init
...
import { looksLikePrivateKey } from "@catalogus/schema";
...
export function privateFlagRefusalMessage(flag: string): string {
  return (
    ...
    "catalogus.yaml is Layer 2 -- committed to the repo -- and must never store that. " +
    'Once the private overlay is available, use "catalogus push --private" instead.'
  );
}
...
// entirely in @catalogus/schema's free-text-guard.ts, which validateManifest
// also runs, so a hand-edited catalogus.yaml gets the identical protection a
...
// practice that made the write-time gate *stricter* than `catalogus
...
// no override flag -- the only way around it was hand-editing catalogus.yaml,
...
// surfaces as a warning, via the same `warnings` channel `catalogus validate`
...
// swallowed either.
import { scanFreeTextForPrivateValues } from "@catalogus/schema";
